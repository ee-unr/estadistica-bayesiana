...
1.  **Los puntos uniformes**
    ...
    Una forma de simular puntos con distribución uniforme dentro de un círculo de radio $r$ es la
    siguiente:

    1. Elegir un ángulo $\theta$ al azar, uniformemente en $[0, 2\pi)$.
    2. Elegir una distancia $\delta$ al centro del círculo, de manera adecuada para garantizar
    uniformidad en el área.
    ...
    **Nota** : Para que los puntos queden uniformemente distribuidos en el área del círculo,
    $\delta$ no debe elegirse uniformemente en $[0, r]$, sino como $\delta = r \times \sqrt{U}$,
...
<!-- 1.  **Demostración sobre la distribución estacionaria**
    ...
    la distribución objetivo $p(\boldsymbol{\theta} \mid \boldsymbol{y})$ -->
...
1.  📌 **MCMC con RStan: Precalentamiento (I)**
    ...
    i. $Y \mid \pi \sim \text{Binomial}(\pi, 20)$ con $\pi \sim \text{Beta}(1, 1)$.
    i. $Y \mid \lambda \sim \text{Poisson}(\lambda)$ con $\lambda \sim \text{Gamma}(4, 2)$.
    i. $Y \mid \mu \sim \text{Normal}(\mu, 1^2)$ con $\mu \sim \text{Normal}(0, 10^2)$.
    <!-- @Johnson2021 6.11 -->
...
1.  📌 **MCMC con RStan: Precalentamiento (II)**
    ...
    i. $Y \mid \pi \sim \text{Binomial}(\pi, 20)$ con $\pi \sim \text{Beta}(1, 1)$ e $y = 12$.
    i. $Y \mid \lambda \sim \text{Poisson}(\lambda)$ con $\lambda \sim \text{Gamma}(4, 2)$ e 
    $y = 3$.
    i. $Y \mid \mu \sim \text{Normal}(\mu, 1^2)$ con $\mu \sim \text{Normal}(0, 10^2)$ e 
    $y = 12.2$.
    <!-- @Johnson2021 6.12 -->

1.  📌 **Modelo Beta-Binomial con RStan (I)**

    Considere el modelo Beta-Binomial para $\pi$ con $Y \mid \pi \sim \text{Binomial}(\pi, n)$
...
1.  📌 **Modelo Normal-Normal con RStan (I)**
    ...
    $Y_i \mid \mu \sim \text{Normal}(\mu, 1.3^2)$ y $\mu \sim \text{Normal}(10, 1.2^2)$.
...
1.  📌 **Modelo Normal-Normal con RStan (II)**
    ...
    $Y_i \mid \mu \sim \text{Normal}(\mu, 8^2)$ y $\mu \sim \text{Normal}(-15, 2^2)$.
...
1.  Considere la siguiente familia de distribuciones normales en 2D
    ...
    $$
    f(\boldsymbol{x} \mid \boldsymbol{\Sigma}, \boldsymbol{\mu} = \boldsymbol{0}) 
