...
logarítmica. Pero, ¿de qué sirve tener una herramienta tan potente si no está claro cómo dominar
a la bestia?
...
por las calles del pueblo a subirse a un Fórmula 1 en pleno Gran Premio.
...
antes de cada Gran Premio, corren entre 400 y 500 vueltas en el circuito... ¡pero en un simulador!
...
2. Utilice Metropolis-Hastings para obtener muestras de $X$. Para ello, obtenga muestras de $Y$
utilizando una propuesta normal con $\sigma = 0.2$ y realice la transformación correspondiente para
obtener las muestras de $X$. Para evaluar la bondad del método, calcule el tamaño de muestra
...
y compárelo con el obtenido anteriormente. Finalmente, obtenga un histograma de las muestras
y compárelo con la densidad empírica y con el histograma obtenido en el punto anterior.

### 2. _Free practice_: múltiples dimensiones

Al computo en escala logarítmica y el uso de transformaciones de variables, en esta etapa se le
...
logarítmica, sin normalizar. Esta función tendrá 2 parámetros de entrada, uno para el vector de
parámetros y otro para el vector de valores observados.
...
$\boldsymbol{\theta} = \{\mu, \sigma\}$. Utilice dos cadenas de Markov. Calcule el tamaño efectivo
...
`metropolis_hastings_log()` debe funcionar al ser llamada con un único argumento, que es el vector
de parámetros. Pero la función que implementamos en el punto 4 tiene 2 argumentos, uno para
el vector de parámetros y otro para el vector de datos... ¿cómo hacemos?

La función `partial()` de la librería `purrr` permite pre-llenar argumentos de una función.
Toma como entrada una función, los argumentos a pre-llenar, y devuelve una nueva función que hace
lo mismo que la función original, pero con valores de argumentos ya especificados.
