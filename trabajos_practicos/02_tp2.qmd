...
de densidad (o de masa de probabilidad) $p^*(\boldsymbol{\theta})$ en cualquier valor de 
$\boldsymbol{\theta}$, incluso cuando $p^*(\boldsymbol{\theta})$ sea impropia 
(es decir, incluso aunque sea desconocida la constante de normalización que hace que la integral en
el soporte de la función sea igual a uno).

Resúmen del algoritmo:

ESCRIBIR

<!-- NOTA: Por ahora no lo incluyo porque creo que extiende el TP de manera innecesaria -->
...
## Metropolis-Hastings en $\mathbb{R}^p$

<!-- NO SE SI PONDRÍA ESTA SECCIÓN -->
...
Que elegancia la de Francia. Solo se necesitan unas pocas modificaciones para que nuestra descripción
inicial del algoritmo de Metropolis-Hastings pueda ser utilizado para obtener muestras de cualquier
distribución, sin distinción de dimensionalidad y soporte... Pero, la vida es más compleja de 
lo que parece.

## Metropolis-Hastings en escala logarítmica

Sin importar cuan moderna y potente sea nuestra computadora, aún tenemos que convivir con 
el Talón de Aquiles del cómputo informático: los problemas de subdesorbamiento y sobredesbordamiento
(_underflow_ y _overflow_ en inglés). 
Si intentamos multiplicar 100 números en el orden de `0.0001` en R, nos encontraremos que para
la computadora esto es igual a 0, aunque no sea cierto.

El problema del _underflow_ se presenta todo el tiempo en el cómputo estadístico.
Por ejemplo, para evaluar una función de verosimilitud se multiplican tantas densidades como 
observaciones se está tabajando. Dado que estos números suelen ser muy pequeños, no es raro
encontrarse con un subdesbordamiento.
Cuando mayor sea la cantidad de observaciones y la dimensionalidad de la distribución objetivo, 
mayor es el riesgo de encontrarse con un problema de _underflow_.

El riesgo de _underflow_ se incrementa aún más cuando usamos algoritmos del estilo de 
Metrpolis-Hastings no solo porque la densidad objetivo se evalúa miles de veces en diferentes 
puntos del espacio paramétrico, sino que también puede estar dada por la multiplicación de muchos números pequeños
como sucede cuando se realiza inferencia bayesiana.

Si nuestro algoritmo de muestreo se encuentra con un problema de _underflow_, podrían suceder dos cosas:

* Se obtiene un error que detiene la ejecución del programa.
* Se realiza una nueva propuesta, ignorando la propuesta problemática, pero se "rompe" la cadena de Markov.

¿Y qué hacemos? ¿Cuál es la solución? ... Hacer todos los cálculos en escala logarítmica.

TODO: Esribir MH en escala logarítmica.
...
## Ejemplo: Los Colapinto de Bayes

La función `metropolis_hastings()` es una herramienta poderosa que, en teoría, permite obtener 
muestras de cualquier distribución _a posteriori_ que se pueda implementar en R en escala logarítmica.
Pero, ¿de qué sirve tener una herramienta tan potente si no está claro cómo usarla?
...
A nivel del suelo, el ozono es un contaminante atmosférico, 
...
en dos ambientes distintos, el primero rico en ozono y el segundo libre de ozono. 
...
1.  Explorar los datos. ¿Encuentra datos atípicos? ¿Qué consecuencias tendrían en la comparación de los incrementos medios de peso?
