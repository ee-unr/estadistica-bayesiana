---
title: "11 - Aproximación de grilla en 2 dimensiones"
execute:
  fig-align: center
  fig-height: 5
  fig-width: 7
...
grid_a <- seq(0.6, 1.4, length.out = 60)
grid_b <- seq(-2.2, -1.6, length.out = 60)
...
ggplot(posterior_a_df) +
    geom_segment(aes(x = a, xend = a, y = 0, yend = p), color = "#3b78b0") +
    geom_point(aes(x = a, y = p), color = "#3b78b0") +
...
ggplot(posterior_b_df) +
    geom_segment(aes(x = b, xend = b, y = 0, yend = p), color = "#3b78b0") +
    geom_point(aes(x = b, y = p), color = "#3b78b0") +
