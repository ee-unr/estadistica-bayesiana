...
$$
\begin{aligned}
  Y_i \mid \lambda & \sim  \mathrm{Po}(\lambda)\\
  \lambda & \sim  \mathrm{Gamma}(s, r)
\end{aligned}
$$
...
\small{
  \begin{aligned} 
  p(y_i\mid \mu) &= \frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{(y_i-\mu)^2}{2\sigma^2}} \rightarrow \\ 
  p(\mathbf{y}\mid \mu) &= \left(\frac{1}{2\pi\sigma^2}\right)^{n/2}  e^{-\frac{\sum_i(y_i-\mu)^2}{2\sigma^2}} \propto e^{-\frac{(\bar{y}-\mu)^2}{2\sigma^2/n}}
  \end{aligned}
}
$$
...
& \propto e^{\left[ \frac{ 2\bar{y} \mu - \mu^2}{2\sigma^2/n} + \frac{-\mu^2 + 2\mu\theta}{2\tau^2} \right]} \\
& \propto e^{\left[ \frac{(2\bar{y} \mu - \mu^2)n\tau^2 + (-\mu^2 + 2\mu\theta)\sigma^2}{2\sigma^2\tau^2} \right]}
\end{aligned}
$$
...
Descomponemos $\tilde{y}$ en dos partes:

$$\tilde{y} = \mu + (\tilde{y} - \mu) = \mu + \varepsilon$$

. . .

$\tilde{y} \mid \mu$, al igual que $y \mid \mu$, tiene distribución $\mathcal{N}(\mu,\sigma^2)$ (es la verosimilitud), entonces: 

. . .

$$(\tilde{y} - \mu) \mid \mu \;\sim\; \mathcal{N}(0,\,\sigma^2) 
\qquad \leftarrow \text{error del modelo, dado } \mu$$

. . .

$$\mu \mid \mathbf{y} \;\sim\; \mathcal{N}(\theta_n,\,\tau_n^2)
\qquad\qquad \leftarrow \text{posterior de } \mu$$

. . .

$\tilde{y}$ es la suma de dos normales independientes (condicionalmente en $\mu$):

$$\tilde{y} \mid \mathbf{y} \;\sim\; \mathcal{N}(\theta_n,\;\sigma^2 + \tau_n^2)$$
