...
-   Generar $S$ muestras independientes $x^{(s)}$ de una distribución de probabilidad $p$ (con función de densidad $p(x)$)
...
Para el primer problema, sabemos que si $X \sim p$, bajo ciertas condiciones podemos aproximar
...
2.  Para cada valor de la grilla podríamos calcular el _posterior_ sin normalizar haciendo el producto del _prior_ por el _likelihood_ (necesitamos la muestra).
