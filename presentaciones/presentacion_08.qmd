...
Sabemos que la distribución beta se puede parametrizar en términos de su moda $\omega$ y su concentración $\kappa$
...
El valor de $\pi$ depende del valor de $\omega$. Lo sabíamos, después de todo, $\omega$ y $\kappa$ son las _constantes de ajuste del prior_ o _hiperparámetros_. $\kappa$ refleja el grado de credibilidad _a priori_ sobre los valores de $\pi$ (alrededor de $\omega$); controla qué tan concentrada está la distribución de $\pi\mid\omega$ alrededor de $\omega$. En ese sentido, mide qué tan fuertemente creemos que $\pi$ debería parecerse a $\omega$.
...
plot_prior_pi_omega1 <- ggplot(prior |> filter(omega == 0.9)) +
  geom_line(aes(x=pi, y=prior)) +
  xlab(expression(pi))

plot_prior_pi_omega2 <- ggplot(prior |> filter(omega == 0.6)) +
  geom_line(aes(x=pi, y=prior)) +
  xlab(expression(pi))
```
...
Los datos dicen _la moneda parece tener $\pi$ cerca de $\frac{1}{3}$_. El prior jerárquico dice _\pi debería estar alrededor de algún \omega, y antes de ver datos creíamos que \omega estaba cerca de \frac{1}{2}_. Como la asociación es débil/moderada, $\pi$ se mueve hacia los datos, pero $\omega$ solo se mueve un poco hacia abajo.

- $\pi$ aprende bastante de los datos
- $\omega$ aprende poco, porque solo recibe información a través de $\pi$
...
prior_pi_1 <- ggplot(prior |> filter(omega == 0.9)) +
  geom_line(aes(x=pi, y=prior)) +
  xlab(expression(pi))

prior_pi_2 <- ggplot(prior |> filter(omega == 0.6)) +
  geom_line(aes(x=pi, y=prior)) +
  xlab(expression(pi))
```
...
plot_prior + (prior_pi_1/prior_pi_2)
```
...
La estructura jerárquica transmite mucha más información por la asociación fuerte de $\pi$ y $\omega$. Si los datos sugieren que $\pi\approx\frac{1}{3}$, eso también se vuelve evidencia fuerte de que $\omega$ debería estar más cerca de $\frac{1}{3}$.
...
La evidencia dice $\pi$ parece estar cerca de $\frac{1}{3}$. Pero el _prior_ dice muy fuertemente $\omega$ está cerca de $\frac{1}{2}$. Como $\kappa$ no es grande, el modelo puede acomodar ambas cosas: permite que $\pi$ se aleje de $\omega$ sin tener que mover mucho a $\omega$.

- _Observé una moneda que parece sesgada, pero sigo creyendo que el proceso de fabricación apunta a monedas justas; puede haber salido una moneda particular desviada_

##

En los tres casos la verosimilitud es la misma: los datos informan directamente sobre $\pi$.
Lo que cambia es cómo esa información se transmite hacia $\omega$: por la fuerza de asociación $\kappa$ y por la fuerza del _prior_ $p(\omega)$.
...
Con $y_1$ caras en $N_1$ tiradas de la moneda 1 estimamos $\pi_1$, con $y_2$ caras en $N_2$ tiradas de la moneda 2 estimamos $\pi_2$... y luego, con todas las tiradas, podemos estimar $\omega$.
...
$$
\begin{aligned}
    Y_i\mid\alpha_{j[i]},\sigma & \sim  \mathcal{N}(\alpha_{j[i]},\sigma^2) \\
    \alpha_{j} & = \alpha  \\
    \alpha & \sim  \mathcal{D}_\alpha(\theta_1) \\
    \sigma & \sim  \mathcal{D}_\sigma(\theta_2)
\end{aligned}
$$
...
\begin{aligned}
    Y_i\mid\alpha_{j[i]},\sigma & \sim  \mathcal{N}(\alpha_{j[i]},\sigma^2)\\
    \alpha_j & \sim \mathcal{D}_{\alpha_j}(\theta_1) \\
    \sigma & \sim \mathcal{D}_\sigma(\theta_2)
\end{aligned}
$$

Decimos que los $Y_i$ tienen distribución de media $\alpha_{j[i]}$, sin imponer ninguna restricción sobre los $\alpha_j$. $\mathcal{D}_{\alpha_j}(\theta_1)$ es una distribución no informativa (muy ancha y chata). Todos los $\alpha_j$ son independientes. Coincide con la estimación clásica que incluye una variable _dummy_ para cada grupo.
...
\begin{aligned}
    ...
    \sigma & \sim \mathcal{D_\sigma}(\theta)
\end{aligned}
$$
...
$$
\begin{aligned}
    Y_i\mid\alpha_{j[i]},\sigma & \sim  \mathcal{N}(\alpha_{j[i]},\sigma_y^2) \\
    \alpha_j \mid \mu_\alpha,\sigma_\alpha & \sim  \mathcal{N}(\mu_\alpha,\sigma_\alpha^2) \\
    \mu_\alpha & \sim  \mathcal{D_{\mu_\alpha}}(\theta_1) \\
    \sigma_\alpha & \sim  \mathcal{D_{\sigma_\alpha}}(\theta_2) \\
    \sigma_y & \sim  \mathcal{D_{\sigma_y}}(\theta_3) \\
\end{aligned}
$$
...
$\left[\begin{array}{c}\alpha_j\\\beta_j\end{array}\right]$ tienen una distribución \textit{a priori} conjunta, normal multivariada de hiperparámetros $\left[\begin{array}{c}\mu_\alpha\\\mu_\beta\end{array}\right]$ y $\Sigma$. ¡Necesitan _hiperpriors_}!
