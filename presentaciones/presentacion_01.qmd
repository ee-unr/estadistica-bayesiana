...
> _La probabilidad no existe._ (De Finetti, 1970)
...
> _El razonamiento probabilístico —que siempre debe entenderse como subjetivo— surge meramente de nuestra incertidumbre sobre algo. No importa si la incertidumbre se refiere a un futuro imprevisible, o a un pasado inadvertido... o incluso a algo más o menos conocible pero para lo cual no estamos dispuestos o no podemos hacer el esfuerzo._ (De Finetti, 1974)
...
* **Incertidumbre epistémica**: incertidumbre **irreducible**, inherente al sistema que estamos observando.
* **Incertidumbre aleatoria**: incertidumbre **reducible** que surge porque nuestro modelo es imperfecto o porque no tenemos datos suficientes.
...
Cuando encontramos la cantidad de bolas azules que nos hace indiferentes, esa razón azules/totales **es** nuestra probabilidad subjetiva.
...
- Que el Inter Miami le gane al NYC FC
- Que el Inter Miami le gane al NYC FC y Messi haga un gol
...
Para el primer caso: $p(H_1 \mid \text{rechazo }H_0) = \frac{{0.80}\cdot {0.50}}{{0.05}\cdot {0.50} + {0.80}\cdot {0.50}} = {0.94}$

Para el segundo caso: $p(H_1 \mid \text{rechazo }H_0) = \frac{{0.80}\cdot {0.10}}{{0.05}\cdot {0.90} + {0.80}\cdot {0.10}} = {0.64}$
